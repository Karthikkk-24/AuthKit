...
interface User { id: string; email: string; name: string | null; isLocked: boolean; deletedAt: string | null; emailVerifiedAt: string | null; createdAt: string; lastLoginAt: string | null; role: { name: string } | null; }
interface UsersResponse { data: User[]; total: number; page: number; limit: number; pages: number; }
...
function StatusBadge({ user }: { user: User }) {
  if (user.deletedAt) return <span className="px-2 py-0.5 text-xs rounded-full bg-red-500/20 text-red-400 border border-red-500/30">Deleted</span>;
...
export default function UsersPage() {
  ...
  const lockMutation = useMutation({
    mutationFn: ({ id, lock }: { id: string; lock: boolean }) =>
      ...
      lock
        ? apiClient(`/users/${id}/lock`, {
            method: 'POST',
            body: JSON.stringify({ reason: 'Locked by admin' }),
          })
        : apiClient(`/users/${id}/unlock`, { method: 'POST' }),
    ...
    <div className="space-y-6">
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-700/30">
            {isLoading ? Array.from({ length: 8 }).map((_, i) => (
              <tr key={i}>{Array.from({ length: 6 }).map((_, j) => <td key={j} className="px-6 py-4"><div className="h-4 bg-slate-700/50 rounded animate-pulse" /></td>)}</tr>
            )) : users.length === 0 ? (
              <tr><td colSpan={6} className="px-6 py-16 text-center text-slate-500"><UserIcon className="h-12 w-12 mx-auto mb-3 opacity-30" /><p>No users found</p></td></tr>
            ) : users.map(user => (
              <tr key={user.id} className="hover:bg-slate-700/20 transition-colors">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    {user.emailVerifiedAt && <Check className="h-3.5 w-3.5 text-emerald-400 flex-shrink-0" aria-label="Email verified" />}
                  </div>
                </td>
                <td className="px-6 py-4">
                  {user.role ? <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${ROLE_COLORS[user.role.name] ?? ROLE_COLORS.guest}`}><Shield className="h-3 w-3" />{user.role.name}</span> : <span className="text-slate-500 text-xs">No role</span>}
                </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm">
          <p className="text-slate-400">Showing {(currentPage - 1) * limit + 1}–{Math.min(currentPage * limit, total)} of {total}</p>
